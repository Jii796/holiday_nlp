...
public abstract class AbstractClassifier implements IClassifier
{
    ...
    @Override
    public String[] classify(String text, int len,boolean flag) throws IllegalArgumentException, IllegalStateException
    {
        Map<String, Double> scoreMap = predict(text);
    ...
    @Override
    public int[] label(Document document,boolean flag) throws IllegalArgumentException,IllegalStateException{
        int[] result=new int[10];
        if(flag==false){
            result[0]=label(document)+1;
            return result;
        }
        else{
            int best=-1;
            ...
            double[] probs = categorize(document);
            double[] comp=new double[10];//记录下此时概率最大的标签的概率
            double max = Double.NEGATIVE_INFINITY;
            for (int i = 0; i < probs.length; i++)
            {
                if (probs[i] > max)
            ...
            for(int k=0;k<probs.length-1;k++){
                max=0;
                for(int i=0;i<probs.length;i++){
                    if (probs[i] > max && probs[i]*15>comp[0] &&probs[i]<comp[j])
                    {
                        max = probs[i];
                        result[j+1]=i+1;
                        comp[j+1]=probs[i];
